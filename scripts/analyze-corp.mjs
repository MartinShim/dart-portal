...
const jo = (x) => (x / 1e12).toFixed(1); // 원 → 조원
...
function buildPeriodicAnalysis(item, fin, qoq, div) {
  ...
  if (isAnnual && qoq) {
    keyPoints.push(
      `4분기 단독 — 매출 ${jo(q4("매출액"))}조(QoQ ${qOnly("매출액")}), 영업이익 ${jo(q4("영업이익"))}조(QoQ ${qOnly("영업이익")}), 순이익 ${jo(q4("당기순이익"))}조(QoQ ${qOnly("당기순이익")})`
    );
  }
...
function buildQoQ(reportNm) {
  ...
  const cur = QUARTER_SERIES[`${year}Q${q}`];
